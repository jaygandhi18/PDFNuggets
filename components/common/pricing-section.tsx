...
const listVariant = {
    ...
    visible: {
        opacity: 1, x: 0, transition: { type: 'spring', damping: 20, stiffness: 100 }
    }, 
}

const PricingCard = ({ name, price, description, items, id, paymentLink, priceId }: PriceType) => {
    return <MotionDiv variants={listVariant} whileHover={{scale:1.02}} className="relative w-full max-w-lg hover:scale-105 transition-all duration-300">
        <div className={cn(
            ...
            <MotionDiv variants={listVariant} className="space-y-2.5 leading-relaxed text-base flex-1">
                {items.map((item: any, idx: any) => {
                    return <li className="flex items-center gap-2" key={idx}>
            ...
            <MotionDiv variants={listVariant} className="space-y-2 flex justify-center w-full">
                <Link
                    ...
                    className={cn(
                        ...
                        id === 'pro'
                            ? "border-teal-900"
                            : "border-teal-100 from-teal-400 to-cyan-500"
                    )}
                >
